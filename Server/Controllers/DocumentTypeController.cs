...
namespace Server.Controllers
{
    ...
    {
        [HttpPost]
        public async Task<WebDocumentType> SaveAsync(WebDocumentType cls)
        {
            try
            {
                ...
                var res = await DocumentTypeBussines.SaveAsync(cls, cusGuid);
                if (res.HasError) return null;
                Assistence.SaveLog(cusGuid, cls.Guid, EnTemp.DocumentType);
                return cls;
            }
            catch (Exception ex)
            {
